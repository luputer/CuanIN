...
export default function CheckoutPage() {
    ...
    const renderFormField = (field: FormFieldData) => {
        ...
        switch (field.type) {
            case "SHORT":
                return <input {...register(`custom.${field.id}`)} placeholder={`Masukkan ${field.label.toLowerCase()}`} className={inputClass(!!fieldError)} />;
            case "LONG":
                return <textarea {...register(`custom.${field.id}`)} placeholder={`Masukkan ${field.label.toLowerCase()}`} rows={3} className={inputClass(!!fieldError)} />;
            case "MULTIPLE_CHOICE":
                return (
                    <div className="space-y-2">
                        {options.map((opt, i) => (
                            <label key={i} className="flex gap-2 items-center text-sm text-slate-700">
                                <input type="radio" value={opt} {...register(`custom.${field.id}`)} />
                                {opt}
                            </label>
                        ))}
                    </div>
                            ...
                            return (
                                <label key={i} className="flex gap-2 items-center text-sm text-slate-700">
                                    <input
                                        ...
                                        checked={checked}
                                        onChange={() => {
                                            const newValues = checked ? values.filter((v) => v !== opt) : [...values, opt];
                                            form.setValue(`custom.${field.id}`, newValues.join(","), { shouldValidate: true });
                                        }}
                                    />
                                    {opt}
                                </label>
            ...
            case "DROPDOWN":
                return (
                    <select {...register(`custom.${field.id}`)} className={inputClass(!!fieldError)}>
                        <option value="">Pilih {field.label.toLowerCase()}</option>
                        {options.map((opt, i) => <option key={i} value={opt}>{opt}</option>)}
                    </select>
                );
            default:
                return <input {...register(`custom.${field.id}`)} className={inputClass(!!fieldError)} />;
        }
    };
        ...
        <div className="min-h-screen bg-slate-50">
            ...
            <div className="bg-white border-b border-slate-300 sticky top-0 z-10">
                <div className="max-w-6xl mx-auto px-4 h-16 flex items-center">
                    <Link href={`/${slug}/${productSlug}`} className="w-10 h-10 flex items-center justify-center hover:bg-slate-100 rounded-full">
                    ...
                    <div className="lg:col-span-3 space-y-6 pb-50">
                        ...
                        <div className="bg-white border border-slate-300 rounded-xl p-5 flex gap-5">
                            ...
                            <div className="mt-1 flex flex-col h-full space-y-2">
                                ...
                                <h2 className="text-lg font-bold text-slate-800 mb-4">{product.name}</h2>
                        ...
                        {/* FORM */}
                        <form id="checkout-form" onSubmit={handleSubmit(onSubmit)} className="bg-white border border-slate-300 rounded-xl p-6 space-y-5 shadow-[0_-4px_0px_0px_rgba(0,146,184,100)]">
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="text-sm text-slate-700">Nama <span className="text-red-500">*</span></label>
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="text-sm text-slate-700">No HP <span className="text-red-500">*</span></label>
                                    ...
                                    {formFields.map((field) => (
                                        <div key={field.id} className="flex flex-col gap-2">
                                            ...
                                            {errors?.custom?.[field.id] && (
                                                <p className="text-xs text-red-500">{errors.custom[field.id]?.message?.toString()}</p>
                                            )}
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div className="bg-white border border-slate-300 rounded-xl p-5">
                            ...
                            <div className="flex gap-3 mt-2">
                                <div className="relative flex-1">
                                    ...
                                    <input {...register("promo")} className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-300 focus:border-cyan-600 focus:ring-1 focus:ring-cyan-100" placeholder="Masukkan kode voucher" />
                                </div>
                                <button className="px-5 py-2.5 rounded-xl bg-yellow-300 hover:bg-yellow-400 text-slate-800 text-sm font-medium">Pakai</button>
                            </div>
                        </div>

                        {/* SUMMARY */}
                        <div className="bg-white border border-slate-300 rounded-xl p-6">
                            <h3 className="font-semibold text-slate-800 border-b border-slate-300 pb-3 mb-4">Detail Pembayaran</h3>
                            ...
                            <button
                                ...
                                className="mt-6 w-full py-3 bg-cyan-600 hover:bg-cyan-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl font-medium"
                            >
