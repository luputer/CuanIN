...
function SortableFieldItem({
    ...
    return (
        <div
            ref={setNodeRef}
            style={style}
            className={`bg-white border border-slate-400 rounded-lg py-5 pl-4 pr-5 flex items-center gap-4 ${isDragging ? "shadow-lg ring-2 ring-cyan-500 z-50" : ""}`}
        >
                ...
                <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        <select
                            ...
                            className="border border-slate-300 rounded-md px-3 py-1.5 text-[15px] font-medium text-slate-600 focus:outline-none bg-white pr-8"
                        >
                            ...
                            {field.options?.map((option, index) => (
                                <div key={index} className="flex items-center gap-3">
                                    ...
                                    {field.options!.length > 1 && (
                                        <button onClick={() => removeOption(field.id, index)} className="text-slate-400 hover:text-red-500">
                                            <XIcon className="w-4 h-4" />
                                        </button>
                                    )}
                                </div>
                            ))}
                            <button onClick={() => addOption(field.id)} className="text-[#00B4D8] text-[15px] font-medium">+ Tambah Opsi</button>
                        </div>
                    )}
                </div>
...
export function FormCustomizer({ productId }: { productId: string }) {
    ...
    const addField = () => setFields([...fields, { id: crypto.randomUUID(), label: "", type: "SHORT", required: false }]);
