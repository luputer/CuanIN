...
export function PortalPurchaseCard({ purchase, isHistoryTab }: { purchase: PortalPurchaseType; isHistoryTab: boolean }) {
  ...
  const detailUrl = isHistoryTab
    ? `/portal/dashboard/riwayat/${purchase.id}`
    : `/portal/dashboard/produk/${purchase.id}`;

  return (
    <div className="relative h-full flex flex-col overflow-hidden rounded-xl border border-slate-300 bg-white">
      {/* Clickable Area (Thumbnail and Info) */}
      <Link href={detailUrl} className="flex-1 flex flex-col px-4 pt-4 pb-0 group cursor-pointer">
        {/* Thumbnail */}
        <div className="relative flex aspect-square w-full items-center justify-center overflow-hidden rounded-xl bg-slate-100">
          {product.image ? (
            <Image src={product.image} alt={product.name} fill className="object-cover group-hover:scale-[1.02] transition-transform duration-200" unoptimized />
          ) : (
        ...
        <div className="flex-1 flex flex-col justify-between space-y-1.5 pt-4">
          <div>
            ...
            <p className="mb-1 line-clamp-2 text-sm leading-snug font-semibold text-slate-800 group-hover:text-cuan-cyan transition-colors">
              {product.name}
            </p>
      ...
      {/* Non-Clickable Action Buttons Area */}
      <div className="px-4 pb-4 pt-2 mt-auto">
        ...
        {isHistoryTab && (
          <div className="flex items-center gap-2">
            <Link
              href={detailUrl}
              className="flex-1 flex items-center justify-center gap-1.5 rounded-md border border-cuan-cyan py-2 px-4 text-sm font-semibold text-cuan-cyan transition-colors duration-200 hover:bg-cuan-cyan/10"
            >
              Lihat Detail
            </Link>
